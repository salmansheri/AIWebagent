  ...
  const mutation = useMutation({
    mutationFn: async (message: string) => {
      const apiResponse = await fetch(
        'http://localhost:5071/api/AIWebAgent/ask',
        {
          method: 'POST',
          headers: {
        ...
        {isOpen && (
          ...
          >
            <div className="flex justify-between items-center mb-2">
              <h2 className="text-lg text-gray-900 font-bold">AI Web Agent</h2>
            ...
            <div className="flex w-full items-center mt-2 gap-2">
              <input
                className="w-full text-amber-900 p-2 rounded-xl focus:ring-1 ring-amber-900 focus:outline-none "
                placeholder="chat"
              />
              <button
